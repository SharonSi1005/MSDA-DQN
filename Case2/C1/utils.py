...
def paramInit(df=None, WCL=None, rho_ave=0.7673, q_t=25, alpha=0.15, rho_ch_mu=None, speedWCL_mu=None):
    ...
    for index, row in df.iterrows():
        ...
        # 内外环道路速度互换
        speed_limit_old = row['speed limit (km/h)']
        if speed_limit_old == 50:
            speed_limit = 90
        elif speed_limit_old == 60:
            speed_limit = 100
        elif speed_limit_old == 90:
            speed_limit = 50
        elif speed_limit_old == 100:
            speed_limit = 60
        else:
            speed_limit = speed_limit_old
