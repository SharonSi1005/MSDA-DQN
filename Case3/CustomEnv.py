...
class DynChgEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # 法1：终点确定为node 40，随机选择EV agent的出发位置，可为【除终点以外】的任意节点
        # 法2：终点起点位置都不确定，只要两者不同即可 result = [item for item in my_list if item not in another_list]
        # count = global_var.get_value('count')
        # count = count + 1
        # global_var.set_value('count', count)

        # df = pd.read_excel("StartEndNodes2.xlsx")
        # sample = self.np_random.uniform()
        #
        # if sample <= 0.4:
        #     random_row = df.sample(n=1).iloc[0]
        #     self._startLoc = random_row['StartLoc']
        #     self._endLoc = random_row['EndLoc']
        # else:
