...
class DynChgEnv(gym.Env):
    ...
    def __init__(self, n_DWERs=12, n_nodes=41, chgNodes=None, allNodes=None, df=None):
        ...
        self.WCL = [(1, 13), (13, 1), (12, 23), (23, 12), (9, 21), (21, 9), (26, 27), (27, 26), (37, 38), (38, 37),
                    (6, 7), (7, 6)]
        if allNodes is None:
        ...
        self.beta = 0  # 车主偏好，取值范围[-1,1]
        self.episode_number = 0  # 初始化行程编号
        ...
        self.observation_space = Dict({"loc": Discrete(n_nodes), "SOC": Box(low=0, high=1, dtype=np.float64),
    ...
    def reset(self, seed=None, options=None):
        ...
        self._loc = self._startLoc
        self._SOC = np.array([self.np_random.uniform(low=0.3, high=0.7, )])
        # print("SOC=", self._SOC)
        self._curTime = np.zeros(1)

        G_velocity, rho_ch_s, Vmn, G_length_adj_matrix, G_velocity_adj_matrix, G_eqv_adj_matrix = \
            utils.paramInit(df=self.df)
        ...
        if hasattr(self, "Se"):
            self.beta = 1 - 2 / (1 + np.exp(-10 * (self.Se - 0.5)))

        # print("beta=", self.beta)

        Ctr, L, Cch, deta_e_left = utils.getFeatures(
            rho_ch_s,
            Vmn,
            G_length_adj_matrix,
            G_eqv_adj_matrix,
    ...
    def step(self, action):  # action={0,1,2,...,11}
        ...
        if lastLoc != self._loc:
            delta_T_hour = G_length_adj_matrix[lastLoc, self._loc] / G_velocity_adj_matrix[lastLoc, self._loc]

            if np.isnan(delta_T_hour):
                print("时间更新出错！")
                print("出错的边为：", (lastLoc, self._loc))
                print("action=", self.chgNodes[action])
                print("L2=", self._L2)
                print("length=", G_length_adj_matrix[lastLoc, self._loc])
                print("speed=", G_velocity_adj_matrix[lastLoc, self._loc])
        ...
        self._Cch = np.array(list(Cch.values()))
        self._L2 = np.empty([self.n_DWERs, 2], dtype=int)
        deta_e_left_array = np.array(list(deta_e_left.values()))
        idx = 0
        for key in L.keys():
            if len(L[key]) >= 2:
                self._L2[idx, 0] = L[key][0]
        ...
        if terminated:
            ...
            # 本次导航结束，用户给出满意度评分
            self.Se = (et - deta_e_left_array[action]) / self.emax

            # print("电量满意度：", self.Se)

            info_step = {"custom_info": {"episode": self.episode_number,
                                         "Se": self.Se,
                                         "beta": self.beta}}    # 当前行程编号
            # print(f"Step Info: {info_step}")

        else:
            reward = -self.Wij[lastLoc, self._loc]  # reward用上一时刻的G_eqv_adj_matrix算
        ...
        return observation, reward, terminated, truncated, info_step
