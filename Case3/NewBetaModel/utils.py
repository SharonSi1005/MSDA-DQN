...
def getFeatures(
    ...
    L_left = {}  # 从WCL k的终点到end node的最优路径
    em = {}  # 到达WCL起点时的电量
    deta_e_left = {}  # 从WCL k的终点到end node消耗的电量
    ech_max = {}  # 每个WCL最大充电量
    ech = {}  # 实际充电量
    ...
    for k in range(12):
        ...
        L_left[Nch[k]] = []
        v = Nch[k]
        while math.isnan(prev2[v]) is False:
            L_left[Nch[k]].insert(0, v)
            v = prev2[v]

        deta_e_left[Nch[k]] = alpha * sum(
            [G_length_adj_matrix[L_left[Nch[k]][i], L_left[Nch[k]][i + 1]] for i in range(len(L_left[Nch[k]]) - 1)]
            , G_length_adj_matrix[end_node, L_left[Nch[k]][0]])
...
if __name__ == "__main__":
    ...
    Ctr, L, Cch, deta_e_left = getFeatures(
        rho_ch_s,
        Vmn,
        G_length_adj_matrix,
        G_eqv_adj_matrix,
