...
class DynChgEnv(gym.Env):
    ...
    def __init__(self, n_DWERs=12, n_nodes=41, chgNodes=None, allNodes=None, df=None,
        ...
        self.SOC_real = np.array([0.5], dtype=np.float64)
        self.SOC_measured = np.array([0.5], dtype=np.float64)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.SOC_real = np.array([self.np_random.uniform(low=0.4, high=0.6, )])
        self.SOC_measured = self.SOC_real
        self._SOC = self.SOC_real
    ...
    def step(self, action):  # action={0,1,2,...,11}
        ...
        et_real = self.SOC_real * self.emax - self.alpha * G_length_adj_matrix[lastLoc, self._loc]

        # 模拟测量值
        if np.random.rand() < self.error_probability:
            # 随机选择正误差或负误差
            sign = 1 if np.random.rand() < 0.5 else -1
            et_measured = et_real * (1 + sign * self.error_percentage)
        else:
            et_measured = et_real

        self.SOC_real = et_real / self.emax
        self.SOC_measured = et_measured / self.emax
        self._SOC = self.SOC_measured
