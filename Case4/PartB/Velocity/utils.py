...
def paramInit(df=None, WCL=None, rho_ave=0.7673, q_t=25, alpha=0.15, rho_ch_mu=None, speedWCL_mu=None,
    ...
    G_length = nx.Graph()
    G_velocity_real = nx.Graph()
    G_velocity_measured = nx.Graph()
    G_eqv_real = nx.Graph()
    G_eqv_measured = nx.Graph()
    ...
    for index, row in df.iterrows():
        ...
        velocity_real = velocity.rvs(1)[0]  # 用截断正态分布随机生成每条路段的【真实】车速vij，sample size=1
        # 模拟测量值
        if np.random.rand() < error_probability:
            # 随机选择正误差或负误差
            sign = 1 if np.random.rand() < 0.5 else -1
            velocity_measured = velocity_real * (1 + sign * error_percentage)
        else:
            velocity_measured = velocity_real

        # 真实值相关graph
        G_velocity_real.add_edge(source_node, target_node, weight=velocity_real)
        weight_eqv_real = alpha * rho_ave * length + q_t * length / velocity_real  # 计算等效边权重
        G_eqv_real.add_edge(source_node, target_node, weight=weight_eqv_real)

        # 测量值相关graph
        G_velocity_measured.add_edge(source_node, target_node, weight=velocity_measured)
        weight_eqv_measured = alpha * rho_ave * length + q_t * length / velocity_measured  # 计算等效边权重
        G_eqv_measured.add_edge(source_node, target_node, weight=weight_eqv_measured)

    G_length_adj_matrix = nx.to_numpy_matrix(G_length, nodelist=list(range(0, 41)))

    G_velocity_adj_matrix_real = nx.to_numpy_matrix(G_velocity_real, nodelist=list(range(0, 41)))
    G_velocity_adj_matrix_measured = nx.to_numpy_matrix(G_velocity_measured, nodelist=list(range(0, 41)))

    G_eqv_adj_matrix_real = nx.to_numpy_matrix(G_eqv_real, nodelist=list(range(0, 41)))
    G_eqv_adj_matrix_measured = nx.to_numpy_matrix(G_eqv_measured, nodelist=list(range(0, 41)))
    ...
    return G_velocity_measured, rho_ch_s, Vmn, G_length_adj_matrix, \
...
def getFeatures(
    ...
    g = Graph(41)
    g.graph = G_eqv_adj_matrix_measured
...
if __name__ == "__main__":
    ...
    G_velocity_measured, rho_ch_s, Vmn, G_length_adj_matrix, \
        G_velocity_adj_matrix_real, G_velocity_adj_matrix_measured, \
        G_eqv_adj_matrix_real, G_eqv_adj_matrix_measured = paramInit()
