...
def paramInit(df=None, WCL=None, rho_ave=0.7673, q_t=25, alpha=0.15, rho_ch_mu=None, speedWCL_mu=None):
    ...
    if rho_ch_mu is None:
        # rho_ch_mu = [0.4, 0.4, 0.5, 0.5, 0.6, 0.6, 0.5, 0.5, 0.3, 0.3, 0.7, 0.7]
    ...
    # if speedWCL_sigma is None:
    # speedWCL_sigma = [8, 8, 6, 6, 7, 7, 8, 8, 5, 5, 7, 7]
    ...
    for index, row in df.iterrows():
        ...
        velocity_s = velocity.rvs(1)[0]  # 用截断正态分布随机生成每条路段的车速vij，sample size=1
        # print(velocity_s)
...
def getFeatures(
        ...
        Nch=None,
        WCL=None,
        EnableBlockWCLs=False,
        EnableDynPrices=False,
        EnableTollGPLs=False,
    ...
    '''如果Vmn<20，G_eqv_adj_matrix[m,n]+拥堵费'''
    g = Graph(41)

    if EnableTollGPLs is True:
        # 遍历每条可充电路段
        for (entrance, exit), speed in Vmn.items():
            if speed < Vmin:
                # print(f"路段 {(entrance, exit)} 的速度 {speed} < 20，更新与入口节点 {entrance} 相关的边")
                # entrance 作为列索引，即更新所有指向 entrance 的边
                # 找出矩阵中所有行 i，使得 G_eqv_adj_matrix[i, entrance] > 0
                row_indices = np.where(G_eqv_adj_matrix[:, entrance] > 0.01)[0]
                for i in row_indices:  # 包括WCL本身（因为有WCL的路段也有GPL）
                    # old_weight = G_eqv_adj_matrix[i, entrance]
                    G_eqv_adj_matrix[i, entrance] += 5
                    # print(f"更新边 {i} -> {entrance}：{old_weight} -> {G_eqv_adj_matrix[i, entrance]}")
    ...
    for k in range(12):
        ...
        if EnableDynPrices is True:
            # 计算充电车道的拥堵程度C
            if Vmn[WCL[k]] < Vmin:
                C = 1 - Vmn[WCL[k]] / Vmin
                Cch_ndarray = 3 * rho_ch_s[WCL[k]] * ech[WCL[k]] * (1 - beta * C)  # 若拥堵，则三倍电价
            else:
                C = 0
    ...
    if EnableBlockWCLs is True:
        BlockWCLs = [(12, 23), (23, 12), (37, 38), (38, 37)]

        for key in BlockWCLs:
            if key in Cch:
                Cch[key] = 100  # 为block的WCL设置一个很大的Cch
